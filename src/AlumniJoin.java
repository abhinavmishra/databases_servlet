...
@WebServlet(description = "Adds a book to the library under the current user", urlPatterns = { "/alumniorg" })
public class AlumniJoin extends HttpServlet  {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try { 
			...
			if(opt.equals("leave")){ 
				String remove_query = "DELETE FROM ALUMNI WHERE UNI='" + uni + "' AND OID=" + oid; 
				...
				String insert_query = "INSERT INTO ALUMNI (UNI,OID,NAME,CONTACT_INFO,JOB_TITLE,COMPANY_NAME) values ('" + uni
						+ "', " + oid  + ", '"+ name+  "', '" + contact_info + "', '" + job_title + "', '" + company_name + "')";
				...
				if(!company_name.equals("")){ 
					String update_query = "UPDATE ALUMNI SET COMPANY_NAME='" + company_name + "' WHERE UNI='" + uni + "' AND OID=" + oid;   
				...
				if(!job_title.equals("")){ 
					String update_query = "UPDATE ALUMNI SET JOB_TITLE='" + job_title + "' WHERE UNI='" + uni + "' AND OID=" + oid;   
					Statement update_stmt = conn.createStatement(); 
					int result = update_stmt.executeUpdate(update_query);
					if(result==0){ 
						request.getSession().setAttribute("badjoinorg", true);
				...
				if(!contact_info.equals("")){ 
					String update_query = "UPDATE ALUMNI SET CONTACT_INFO='" + contact_info + "' WHERE UNI='" + uni + "' AND OID=" + oid;   				
					Statement update_stmt = conn.createStatement(); 
					int result = update_stmt.executeUpdate(update_query);
					if(result==0){ 
						request.getSession().setAttribute("badjoinorg", true);
